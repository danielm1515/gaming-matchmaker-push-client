...
export default function PartiesPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err?.response?.data?.detail ?? 'Failed to create party'),
  });
  ...
  const handleCreate = () => {
    if (!form.game_id || !form.region_id) {
      toast.error('Select a game and region');
      return;
    }
    ...
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="font-display text-3xl font-bold text-text-primary">Open Parties</h1>
        ...
        <div className="card p-4 mb-6 flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <p className="text-xs text-text-secondary mb-2">Filter by Game</p>
            <GameSelector value={gameFilter} onChange={setGameFilter} />
          </div>
          <div className="flex-1">
            <p className="text-xs text-text-secondary mb-2">Filter by Region</p>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-text-muted mb-4">No open parties. Be the first!</p>
      ...
      {/* Create Party Modal */}
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Create a Party" maxWidth="max-w-md">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-text-secondary">Party Name (optional)</label>
            <input
              value={form.name}
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1 flex flex-col gap-1">
              <label className="text-sm font-medium text-text-secondary">Max Size: {form.max_size}</label>
              <input
                type="range"
                min={2}
                max={6}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-text-secondary">Min Skill</label>
              <select
            ...
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-text-secondary">Max Skill</label>
              <select
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              className="accent-accent-primary"
            />
            <span className="text-sm text-text-secondary">Public party (visible to everyone)</span>
          </label>
